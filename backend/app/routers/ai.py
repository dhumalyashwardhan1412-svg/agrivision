...
router = APIRouter(
    prefix="/ai",
    tags=["AI & Computer Vision Intelligence"],
)
...
async def chat_with_agri_guide(
    ...
    """
    AgriVision conversational assistant for crop guidance,
    disease management, soil nutrient advice and market strategies.
    """
...
async def analyze_crop_leaf_disease(
    ...
    """
    Analyzes crop or leaf images to identify possible diseases,
    ...
    filename = file.filename or "crop-image.jpg"

    # Save uploaded image
    file_url = image_service.save_upload_file(
        contents,
        filename,
    )

    # Analyze image
    diagnosis = image_service.diagnose_crop_image(
        file_url,
        crop_hint,
    )
...
@router.post(
    "/analyze-soil-photo",
    response_model=SoilObservationResponse,
)
async def analyze_soil_photo(
    ...
    """
    Performs visual soil analysis for texture, moisture,
    organic matter appearance and preliminary recommendations.

    This does not replace laboratory NPK testing.
    """
    ...
    filename = file.filename or "soil-image.jpg"
    ...
    # IMPORTANT:
    # image_service.py contains analyze_soil_image(),
    # not analyze_soil_photo().
    obs = image_service.analyze_soil_image(file_url)
